...
FlutterVirtualBackground::FlutterVirtualBackground(RTCVideoTrack* track)
    : track_(track) {
  if (track == nullptr) {
    std::cerr << "Error: Received null track in FlutterVirtualBackground constructor." << std::endl;
    throw std::invalid_argument("Received null track in FlutterVirtualBackground constructor");
  }
...
void FlutterVirtualBackground::OnFrame(scoped_refptr<RTCVideoFrame> frame) {
    ...
    targetRawOutput_->setI420Callbck([=](const uint8_t *data, int width, int height, int64_t ts) {
        // std::cout << "Received processed frame callback in I420 format." << std::endl;
    ...
    gpuPixelRawInput->uploadBytes(width, height, data_y, stride_y, data_u, stride_u, data_v, stride_v);
}
